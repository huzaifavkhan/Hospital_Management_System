...
import { UserRound, Eye, EyeOff, Cross } from 'lucide-react';
...
export default function LoginReceptionist() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/auth/login/receptionist', form);
    ...
    <div className="min-h-screen bg-navy flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 bg-gradient-to-br from-navy to-navy-800 border-r border-white/10">
        ...
        <div>
          <div className="w-16 h-16 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center mb-6">
            <UserRound size={32} className="text-blue-400" />
          </div>
          <h1 className="font-heading text-white text-3xl font-bold mb-3">Receptionist Portal</h1>
          <p className="text-white/50 text-base leading-relaxed max-w-sm">
            Manage patient registrations, book appointments, and coordinate daily front-desk operations efficiently.
          </p>
        </div>
        <p className="text-white/20 text-sm">Front-desk staff access</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <p className="text-white/40 text-sm mb-8">Receptionist credentials required</p>
